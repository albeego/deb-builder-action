...
echo "Building version: $INPUT_VERSION"

mkdir -p build/"$INPUT_PACKAGE_NAME"-"$INPUT_VERSION"/debian
git tag --sort=-authordate --format "$INPUT_PACKAGE_NAME (%(refname:lstrip=2)) UNRELEASED; urgency=medium%0a%0a  * %(subject)%0a%0a -- %(authorname) %(authoremail)  %(authordate:rfc2822)" > build/"$INPUT_PACKAGE_NAME"-"$INPUT_VERSION"/debian/changelog
echo 9 > build/"$INPUT_PACKAGE_NAME"-"$INPUT_VERSION"/debian/compat
cp -r debian/* build/"$INPUT_PACKAGE_NAME"-"$INPUT_VERSION"/debian/
sed -i "s/\$VERSION/$INPUT_VERSION/g" build/"$INPUT_PACKAGE_NAME"-"$INPUT_VERSION"/debian/control
