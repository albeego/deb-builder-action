...
git tag --sort=-authordate --format "$INPUT_PACKAGE_NAME (%(refname:lstrip=-1)) UNRELEASED; urgency=medium%0a%0a  * %(subject)%0a%0a -- %(authorname) %(authoremail)  %(authordate:rfc2822)" > build/"$INPUT_PACKAGE_NAME"-"$VERSION"/debian/changelog
